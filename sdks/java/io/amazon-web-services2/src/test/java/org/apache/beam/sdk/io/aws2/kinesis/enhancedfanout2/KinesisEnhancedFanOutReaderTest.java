...
public class KinesisEnhancedFanOutReaderTest {
  @Mock private KinesisIO.Read readSpec;

  @Test
  public void test() throws IOException {
    when(readSpec.getStreamName()).thenReturn("stream-01");
    when(readSpec.getConsumerArn()).thenReturn("consumer-01");
    when(readSpec.getInitialPosition())
        .thenReturn(new StartingPoint(InitialPositionInStream.LATEST));
    ...
    CheckpointGenerator checkpointGenerator =
        new DynamicCheckpointGenerator(
            readSpec.getStreamName(), readSpec.getConsumerArn(), readSpec.getInitialPosition());
